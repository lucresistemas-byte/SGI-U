...
class PosBloc extends Bloc<PosEvent, PosState> {
  ...
  PosBloc() : super(PosState.initial()) {
    ...
    on<ClearSuccess>(
        (event, emit) => emit(state.copyWith(successMessage: null)));
  ...
  Future<void> _onConfirmSale(ConfirmSale event, Emitter<PosState> emit) async {
    ...
    try {
      ...
      // Éxito: limpiar carrito y método de pago conservando la lista de productos
      // (evita el flicker de lista vacía hasta que responda LoadProducts)
      emit(PosState.initial().copyWith(products: state.products));
